# project-docs
Repositorio de documentación del proyecto grupal: casos de uso, scripts de BD, diagramas, entregas y asignaciones. Curso Ingeniería de Software 1 — Universidad Nacional de Colombia, 2026-1.
<img width="1408" height="318" alt="Gemini_Generated_Image_rrrehtrrrehtrrre" src="https://github.com/user-attachments/assets/41fccdc5-2935-4635-a7db-fb1162acf100" />
#  ☀️kERNEL PANIC - Mentora

### El grupo 🖥️😖🖥️:
| integrante | carrera | correo institucional |
|---|---|---|
| GABRIEL BUSTOS MORENO | Ingenieria de Sistemas y Computacion |gbustosm@unal.edu.co|
| PABLO ROSALES OBANDO |Ingenieria de Sistemas y Computaciòn|prosales@unal.edu.co|
| ALEXANDER MOSQUERA RAMIREZ |Ingenieria de Sistemas y Computaciòn|almosquerar@unal.edu.co|
| DIEGO ESTEBAN JIMENEZ GUTIERREZ |Ingenieria de Sistemas y Computaciòn|Djimenezgu@unal.edu.co|

## Descripción del proyecto
#### MENTORA 📋

El sistema **Mentora** busca resolver la desorganización, el ausentismo y la falta de seguimiento en los procesos de tutorías académicas institucionales.

Actualmente, la gestión manual o fragmentada de estos espacios dificulta que:

- Los **estudiantes** encuentren horarios disponibles.
- Los **tutores** planifiquen sus sesiones con base en el historial del alumno.
- Los **administradores** midan la demanda real.

A diferencia de los sistemas de agendamiento genéricos, **Mentora** integra un ciclo académico completo: desde la asignación de cuestionarios por parte de los profesores, hasta el registro de calificaciones y observaciones por parte de los tutores y profesores, centralizando todo en un único flujo de desempeño académico **medible y escalable**.

